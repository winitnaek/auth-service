/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.bsi.sec.util;

/**
 *
 * @author igorV
 */
public final class CacheConstants {

    public static final String SEC_SVC_DATA_NODE = "securityServiceDataNode";

    public static final String ADMIN_METADATA_CACHE = "AdminMetadataCache";
    public static final String AUDIT_LOG_CACHE = "AuditLogCache";
    public static final String COMPANY_CACHE = "CompanyCache";
    public static final String SSO_CONFIGURATION_CACHE = "SSOConfCache";
    public static final String TENANT_CACHE = "TenantCache";
    public static final String TENANT_SSO_CONF_CACHE = "TenantSSOConfCache";
    public static final String GET_COMPANY_CACHE_SEQ = "getCompanyCacheSeq";
}
